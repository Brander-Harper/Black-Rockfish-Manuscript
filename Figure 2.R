...
subadult <- ggplot(subadult_code, aes(x=LO, y=Pieces,fill=LO)) +
  geom_boxplot(outlier.color="grey", alpha = 1.2, outlier.size = 10) +
  scale_fill_manual(values = c("blue", "pink", "cornsilk", "grey", "coral1"))+
  labs(x= "Location", y= "Pieces per fish") +
  theme_classic() +
  theme(axis.text.x = element_text(color = "black", size = 40),
  ...
  stat_summary(fun=mean, geom="point", shape=20, size=20, color="black", fill="black")+ # this pts means on graph
  theme(legend.position="none")

subadult2 <- subadult + geom_hline(yintercept=5, linetype="dashed", color = "purple", size = 8)
...
juvenile <- ggplot(juvenile_code, aes(x=Loc2, y=Pieces, fill=Loc2)) +
  ...
  theme(axis.text.x = element_text(color = "black", size = 40),
        ...
        axis.title.x = element_text(color = "black", size = 70),
        axis.title.y = element_text(color = "black", size = 70)) +
  stat_summary(fun=mean, geom="point", shape=20, size=20, color="black", fill="white")+ # this pts means on graph
  theme(legend.position="none")

juvenile2 <-juvenile + geom_hline(yintercept=1.82, linetype="dashed", color = "purple", size = 8)
plot3 <- (juvenile2 + scale_fill_grey())
plot (plot3)



plot_grid(plot2, plot3, labels = c("A", "B"), label_size = 70, ncol = 1, nrow =2)
