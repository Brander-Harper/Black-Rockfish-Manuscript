...
subadult <- ggplot(subadult_code, aes(x=LO, y=Pieces,fill=LO)) +
  geom_boxplot(outlier.color="black" , alpha = 0.2) +
...
subadult2 <- subadult + geom_hline(yintercept=4.47, linetype="dashed", color = "orange")
plot (subadult2)
...
juvenile <- ggplot(juvenile_code, aes(x=Loc2, y=Pieces, fill=Loc2)) +
  geom_boxplot( outlier.color="black" , alpha = 1.2) +
  labs(x= "Location", y= "Pieces per fish") +
  theme_classic() + 
  stat_summary(fun=mean, geom="point", shape=20, size=4, color="black", fill="white")+ # this pts means on graph
  theme(legend.position="none")

juvenile2 <-juvenile + geom_hline(yintercept=1.82, linetype="dashed", color = "orange")
plot (juvenile2)



plot_grid(subadult2, juvenile2, labels = c("A", "B"), ncol = 1, nrow =2)
